...
//        }
        ...
        session.getUserLive(session.myUserId).observeK(viewLifecycleOwner) { optionalUser ->
            ...
            if (user != null) {
                ...
                val list = user.userId.split("@")
                if (!list.isEmpty()) {
                    userId = list[0]
                }
                views.homeDrawerUserIdView.text = userId

                /* End modified by DUNHGHQ */
            }
        }
        // Profile
        views.homeDrawerHeader.debouncedClicks {
            ...
            navigator.openSettings(
                requireActivity(),
                directAccess = VectorSettingsActivity.EXTRA_DIRECT_ACCESS_GENERAL
            )
        }
        // Settings
        ...
        views.homeDrawerQRCodeButton.debouncedClicks {
            UserCodeActivity.newIntent(requireContext(), sharedActionViewModel.session.myUserId)
                .let {
                    val options =
        ...
        views.homeDrawerInviteFriendButton.debouncedClicks {
            session.permalinkService().createPermalink(sharedActionViewModel.session.myUserId)
                ?.let { permalink ->
        ...
        // Start added by Globits01
//        views.homeDrawerMenuQuotes.debouncedClicks {
//            Log.i("Globits", "button clicked")
//            activity?.let {
//                val intent = Intent(it, RandomQuoteActivity::class.java)
//                it.startActivity(intent)
//                Log.i("Globits", "activity started...")
//            }
//        }
//
//        views.homeDrawerMenuCallTests.debouncedClicks {
//            activity?.let {
//                val intent = Intent(it, TestCallActivity::class.java)
//                intent.putExtra("oauth2Token", session.oauth2Token)
//
//                it.startActivity(intent)
//            }
//        }
//
//        views.homeDrawerMenuDrugs.debouncedClicks {
//            // TODO
//        }
        // End added by Globits01

        // Debug menu
        views.homeDrawerHeaderDebugView.isVisible =
            BuildConfig.DEBUG && vectorPreferences.developerMode()
