...
class LoginViewModel @AssistedInject constructor(
    ...
    companion object : MvRxViewModelFactory<LoginViewModel, LoginViewState> {
        ...
        override fun create(
            ...
            return when (val activity: FragmentActivity =
                (viewModelContext as ActivityViewModelContext).activity()) {
    ...
    override fun handle(action: LoginAction) {
        when (action) {
            ...
            is LoginAction.UpdateSignModeSignIn -> handleUpdateSignModeSignIn(action)
            ...
            is LoginAction.EnrollClient -> _viewEvents.post(LoginViewEvents.OnEnrollingNewClient)
            is LoginAction.ReturnToLogin -> {
                setState {
                    copy(
    ...
    private fun handleLoginWithToken(action: LoginAction.LoginWithToken) {
        ...
        if (safeLoginWizard == null) {
            ...
            currentJob = viewModelScope.launch {
                try {
                    safeLoginWizard.loginWithToken(action.api,action.loginToken)
    ...
    private fun handleUpdateSignModeSignIn(action: LoginAction.UpdateSignModeSignIn) {
        setState {
            copy(
    ...
    private fun handleDirectLogin(
        ...
        currentJob = viewModelScope.launch {
            val data = try {
                //Modify by DUNGHQ
                //val username = "@"+action.username+":"+"matrix.org";
                val username = "@" + action.username + ":" + "matrix.globits.net";
    ...
    private fun handleUpdateHomeserver(action: LoginAction.UpdateHomeServer) {
        val homeServerConnectionConfig =
            homeServerConnectionConfigFactory.create(action.homeServerUrl)
    ...
    private fun getLoginFlow(homeServerConnectionConfig: HomeServerConnectionConfig) {
        ...
        currentJob = viewModelScope.launch {
            ...
            val data = try {
                val api=SessionManager(applicationContext).fetchClinic()
                authenticationService.getLoginFlow(homeServerConnectionConfig, api ?: "")
            } catch (failure: Throwable) {
                _viewEvents.post(LoginViewEvents.Failure(failure))
                setState {
                    copy(
