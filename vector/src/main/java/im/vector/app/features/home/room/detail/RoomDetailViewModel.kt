...
//            val fragment: RoomDetailFragment = (viewModelContext as FragmentViewModelContext).fragment()
            ...
            return when (fragment) {
                is RoomDetailFragment -> fragment.roomDetailViewModelFactory.create(state)
                is TestCallFragment -> fragment.roomDetailViewModelFactory.create(state)
                else -> null
            }
        }
    }
    ...
    private fun handleStartCallWithPhoneNumber(action: RoomDetailAction.StartCallWithPhoneNumber) {
        viewModelScope.launch {
            try {
                val result = DialPadLookup(
                    session,
                    directRoomHelper,
                    callManager
                ).lookupPhoneNumber(action.phoneNumber)
    ...
    private fun handleAddJitsiConference(action: RoomDetailAction.AddJitsiWidget) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val widgetId: String =
                WidgetType.Jitsi.preferred + "_" + session.myUserId + "_" + System.currentTimeMillis()
            ...
            val confId = roomId.substring(1, roomId.indexOf(":") - 1) + widgetSessionId.toLowerCase(
            ...
            val jitsiDomain =
                preferredJitsiDomain ?: stringProvider.getString(R.string.preferred_jitsi_domain)
            ...
            try {
                val widget =
                    session.widgetService().createRoomWidget(roomId, widgetId, widgetEventContent)
    ...
    private fun handleTombstoneEvent(action: RoomDetailAction.HandleTombstoneEvent) {
        val tombstoneContent =
            action.event.getClearContent().toModel<RoomTombstoneContent>() ?: return
        ...
        withState { state ->
            when (state.sendMode) {
                is SendMode.REGULAR -> {
                    when (val slashCommandResult = CommandParser.parseSplashCommand(action.text)) {
                        ...
                        is ParsedCommand.ErrorSyntax -> {
                            _viewEvents.post(
                                RoomDetailViewEvents.SlashCommandError(
                        ...
                        is ParsedCommand.ErrorUnknownSlashCommand -> {
                            _viewEvents.post(
                                RoomDetailViewEvents.SlashCommandUnknown(
                        ...
                        is ParsedCommand.SetMarkdown -> {
                            ...
                            _viewEvents.post(
                                RoomDetailViewEvents.SlashCommandHandled(
                        ...
                        is ParsedCommand.SendPoll -> {
                            room.sendPoll(
                                slashCommandResult.question,
                                slashCommandResult.options.mapIndexed { index, s ->
                                    OptionItem(
                                        s,
                                        "$index. $s"
                                    )
                        ...
                        is ParsedCommand.CreateSpace -> {
                            viewModelScope.launch(Dispatchers.IO) {
                                try {
                                    ...
                                    _viewEvents.post(
                                        RoomDetailViewEvents.SlashCommandResultError(
                                            failure
                                        )
                                    )
                                }
                            ...
                            viewModelScope.launch(Dispatchers.IO) {
                                try {
                                    session.spaceService()
                                        .joinSpace(slashCommandResult.spaceIdOrAlias)
                                } catch (failure: Throwable) {
                                    _viewEvents.post(
                                        RoomDetailViewEvents.SlashCommandResultError(
                                            failure
                ...
                is SendMode.EDIT -> {
                    ...
                    val inReplyTo =
                        state.sendMode.timelineEvent.getRelationContent()?.inReplyTo?.eventId
                    if (inReplyTo != null) {
        ...
        if (maxUploadFileSize == HomeServerCapabilities.MAX_UPLOAD_FILE_SIZE_UNKNOWN) {
            ...
            when (val tooBigFile = attachments.find { it.size > maxUploadFileSize }) {
                null -> room.sendMedias(attachments, action.compressBeforeSending, emptySet())
                    ...
                    RoomDetailViewEvents.FileTooBigError(
        ...
        visibleEventsObservable
            ...
            .subscribeBy(onNext = { actions ->
                val bufferedMostRecentDisplayedEvent =
                    actions.maxByOrNull { it.event.displayIndex }?.event ?: return@subscribeBy
