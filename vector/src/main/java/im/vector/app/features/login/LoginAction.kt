...
sealed class LoginAction : VectorViewModelAction {
    ...
    data class UpdateSignModeSignIn(val signMode: SignMode) : LoginAction()
    data class LoginWithToken(val api:String,val loginToken: String) : LoginAction()
    ...
    object GetClinics:LoginAction()

    // Login or Register, depending on the signMode
    data class LoginOrRegister(val username: String, val password: String,val api:String , val initialDeviceName: String) :
        LoginAction()
    ...
    // enroll a PrEP client
    object EnrollClient : LoginAction()
    data class CheckDuplicate4EnrollClient(val profile: ClientRegistration) : LoginAction()
    data class SubmitEnrollClient(val profile: ClientRegistration) : LoginAction()
    object HandleshowPolicies :LoginAction()
    object ReturnToLogin : LoginAction()
    ...
    data class CheckUserExist(var email: String?, var name:String?, var isNumber:String?):LoginAction()
    data class SendEmailResetPassword(var email:String,var name:String,var isNumber:String):LoginAction()
}
