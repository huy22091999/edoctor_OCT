...
internal class DefaultLegacySessionImporter @Inject constructor(
    ...
    private suspend fun importCredentials(legacyConfig: LegacyHomeServerConnectionConfig) {
        ...
        val sessionParams = SessionParams(
            credentials = Credentials(
                ...
                discoveryInformation = legacyConfig.credentials.wellKnown?.let { wellKnown ->
                    ...
                    if (wellKnown.homeServer?.baseURL != null || wellKnown.identityServer?.baseURL != null) {
                        DiscoveryInformation(
                            homeServer = wellKnown.homeServer?.baseURL?.let {
                                WellKnownBaseConfig(
                                    baseURL = it
                                )
                            },
                            identityServer = wellKnown.identityServer?.baseURL?.let {
                                WellKnownBaseConfig(
                                    baseURL = it
                                )
                            }
                ...
                // edit by Tuan Anh
                oauth2Token = legacyConfig.credentials.oauth2Token
                // end edit
            ),
    ...
    private fun importCryptoDb(legacyConfig: LegacyHomeServerConnectionConfig) {
        ...
        val sessionId =
            legacyConfig.credentials.let { (if (it.deviceId.isNullOrBlank()) it.userId else "${it.userId}|${it.deviceId}").md5() }
        ...
        Realm.getInstance(realmConfiguration).use {
            ...
            it.writeEncryptedCopyTo(
                File(newLocation, realmConfiguration.realmFileName),
                realmKeysUtils.getRealmEncryptionKey(keyAlias)
            )
        }
    }
